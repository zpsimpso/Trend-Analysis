...
customfit <- function(kfolds, iterations) {
  ...
  for (i in 1:iterations) {
    ...
    customspan <- fitspecial.wrp$span
    y[i] <- customspan
    print(i)
  }
  return(y)
...
NO3test21to45 <- foldsfunction(21,45)
# 
# par(mfrow=c(2,1))
# boxplot(test5to20)
# boxplot(test21to30)
...
NO3test5to45 <- data.frame(NO3test5to20,NO3test21to45)
boxplot(NO3test5to45)
NO3test5to45modes <- apply(NO3test5to45, 2, Mode)
NO3test5to45stdevs <- apply(NO3test5to45, 2, sd)

NO3test5to45stats <- data.frame((5:45), NO3test5to45modes, NO3test5to45stdevs)

write.csv(NO3test5to45, "NO35to45.csv")
...
TNtest21to45 <- foldsfunction(21,45)


#make sure Mode is capitalized


TNtest5to45 <- data.frame(TNtest5to20,TNtest21to45)
boxplot(TNtest5to45)
TNtest5to45modes <- apply(TNtest5to45, 2, Mode)
TNtest5to45stdevs <- apply(TNtest5to45, 2, sd)

TNtest5to45stats <- data.frame((5:45), TNtest5to45modes, TNtest5to45stdevs)

write.csv(TNtest5to45, "TN5to45.csv")
...
SRPtest21to45 <- foldsfunction(21,45)


#make sure Mode is capitalized


SRPtest5to45 <- data.frame(SRPtest5to20,SRPtest21to45)
boxplot(SRPtest5to45)
SRPtest5to45modes <- apply(SRPtest5to45, 2, Mode)
SRPtest5to45stdevs <- apply(SRPtest5to45, 2, sd)

SRPtest5to45stats <- data.frame((5:45), SRPtest5to45modes, SRPtest5to45stdevs)

write.csv(SRPtest5to45, "SRP5to45.csv")
...
TPtest21to45 <- foldsfunction(21,45)


#make sure Mode is capitalized


TPtest5to45 <- data.frame(TPtest5to20,TPtest21to45)
boxplot(TPtest5to45)
TPtest5to45modes <- apply(TPtest5to45, 2, Mode)
TPtest5to45stdevs <- apply(TPtest5to45, 2, sd)

TPtest5to45stats <- data.frame((5:45), TPtest5to45modes, TPtest5to45stdevs)

write.csv(TPtest5to45, "TP5to45.csv")
...
TSStest21to45 <- foldsfunction(21,45)


#make sure Mode is capitalized


TSStest5to45 <- data.frame(TSStest5to20,TSStest21to45)
boxplot(TSStest5to45)
TSStest5to45modes <- apply(TSStest5to45, 2, Mode)
TSStest5to45stdevs <- apply(TSStest5to45, 2, sd)

TSStest5to45stats <- data.frame((5:45), TSStest5to45modes, TSStest5to45stdevs)

write.csv(TSStest5to45, "TSS5to45.csv")
...
Cltest21to45 <- foldsfunction(21,45)


#make sure Mode is capitalized


Cltest5to45 <- data.frame(Cltest5to20,Cltest21to45)
boxplot(Cltest5to45)
Cltest5to45modes <- apply(Cltest5to45, 2, Mode)
Cltest5to45stdevs <- apply(Cltest5to45, 2, sd)

Cltest5to45stats <- data.frame((5:45), Cltest5to45modes, Cltest5to45stdevs)

write.csv(Cltest5to45, "Cl5to45.csv")
...
SO4test21to45 <- foldsfunction(21,45)


#make sure Mode is capitalized


SO4test5to45 <- data.frame(SO4test5to20,SO4test21to45)
boxplot(SO4test5to45)
SO4test5to45modes <- apply(SO4test5to45, 2, Mode)
SO4test5to45stdevs <- apply(SO4test5to45, 2, sd)

SO4test5to45stats <- data.frame((5:45), SO4test5to45modes, SO4test5to45stdevs)

write.csv(SO4test5to45, "SO45to45.csv")

############################################

fiveto45foldsSTATS <- data.frame(NO3test5to45stats, TNtest5to45stats, SRPtest5to45stats, TPtest5to45stats, TSStest5to45stats, Cltest5to45stats, SO4test5to45stats)
write.csv(fiveto45foldsSTATS, "fiveto45foldsSTATS.csv")
...
#reshape2 package
TESTNO3test5to45 <- NO3test5to45
TESTNO3test5to45$id <- 1:nrow(TESTNO3test5to45)
meltNO3 <- melt(TESTNO3test5to45, id.vars="id", variable.name="folds", value.name="span")
meltNO3$const <- "NO3"

TESTTNtest5to45 <- TNtest5to45
TESTTNtest5to45$id <- 1:nrow(TESTTNtest5to45)
meltTN <- melt(TESTTNtest5to45, id.vars="id", variable.name="folds", value.name="span")
meltTN$const <- "TN"

TESTSRPtest5to45 <- SRPtest5to45
TESTSRPtest5to45$id <- 1:nrow(TESTSRPtest5to45)
meltSRP <- melt(TESTSRPtest5to45, id.vars="id", variable.name="folds", value.name="span")
meltSRP$const <- "SRP"

TESTTPtest5to45 <- TPtest5to45
TESTTPtest5to45$id <- 1:nrow(TESTTPtest5to45)
meltTP <- melt(TESTTPtest5to45, id.vars="id", variable.name="folds", value.name="span")
meltTP$const <- "TP"

TESTTSStest5to45 <- TSStest5to45
TESTTSStest5to45$id <- 1:nrow(TESTTSStest5to45)
meltTSS <- melt(TESTTSStest5to45, id.vars="id", variable.name="folds", value.name="span")
meltTSS$const <- "TSS"

TESTCltest5to45 <- Cltest5to45
TESTCltest5to45$id <- 1:nrow(TESTCltest5to45)
meltCl <- melt(TESTCltest5to45, id.vars="id", variable.name="folds", value.name="span")
meltCl$const <- "Cl"

TESTSO4test5to45 <- SO4test5to45
TESTSO4test5to45$id <- 1:nrow(TESTSO4test5to45)
meltSO4 <- melt(TESTSO4test5to45, id.vars="id", variable.name="folds", value.name="span")
meltSO4$const <- "SO4"
...
write.csv(THEWHOLEDAMNTHING, "THEWHOLEDAMNTHING1000.csv")
